...
import com.geeksville.gaggle.SummaryListActivity;
 ...
 * 
 */
public class ListFlightsFragment extends AbstractDBListFragment {

	/**
	 * Debugging tag
	 */
	private static final String TAG = "ListFlightsFragment";
...
//	}
	...
	private String timeToString(long timeMsec) {
		...
		String title = datefmt.format(startdate) + " "
		...
		protected String doInBackground(Long... params) {
			try {
				synchronized (coder) { // only one lookup at a time
					...
					double longitude = locs
							.getDouble(locs
									.getColumnIndex(LocationLogDbAdapter.KEY_LONGITUDE));
					List<Address> addrs = coder.getFromLocation(latitude,
							longitude, 1);
	...
	protected BaseAdapter createListAdapter() {
		...
		final int idcol = myCursor
				.getColumnIndex(LocationLogDbAdapter.KEY_ROWID);
...
//		startActivity(i);
	 ...
	 * @throws IOException
	 */
	private File flightToFile(long flightid, String filetype)
			throws IOException {
		// File cacheDir = getCacheDir();
				...
				protected void doInBackground() {
					...
					try {
						...
						showCompletionDialog(
								getString(R.string.igc_stream_write_failed),
								ex.getLocalizedMessage());
						return;
					}

					try {
						String basename = getString(R.string.flight_)
								+ flightid;
						String toastMessage = LeonardoUpload.upload(
								acct.username, acct.password, acct.serverURL,
								gprefs.getCompetitionClass(), basename,
								fileLoc, acct.connectionTimeout,
								acct.operationTimeout);

						if (toastMessage != null && toastMessage.length() == 0)
							toastMessage = context
									.getString(R.string.upload_failed_bad_url);
