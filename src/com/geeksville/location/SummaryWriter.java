...
public class SummaryWriter implements PositionWriter {
	...
	@Override
	public void emitPosition(long time, double latitude, double longitude, float altitude, int bearing, float groundSpeed, float[] accel) {
		if(firstPosition) {
		...
		else
		{
			...
			totalGroundDistance += LocationUtils.LatLongToMeter(lastLatitude, lastLongitude, latitude, longitude);
			lastLatitude = latitude;
			lastLongitude = longitude;
			
			double currentDistanceFromLaunch = LocationUtils.LatLongToMeter(firstLatitude, firstLongitude, latitude, longitude);
